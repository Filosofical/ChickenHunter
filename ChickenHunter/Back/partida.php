...
// session_start(); // Descomenta si realmente necesitas iniciar sesión para esta operación específica.
                  // Si idUsuario viene del cliente y no de $_SESSION, quizás no sea necesario aquí.
...
if ($data !== null) {
    ...
    if ($idUsuario !== null && $dificultad !== null && $puntaje !== 0) {
        ...
        if ($idUsuario === false || $dificultad === false || $puntaje === false) {
            $response = ['status' => 'error', 'message' => 'Tipos de datos inválidos para idUsuario, dificultad o puntaje.'];
        } else {
         
            try {
         
                 $sql = "INSERT INTO partidas (usuario_id, puntuacion, dificultad ) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
            if ($stmt) {
                $stmt->bind_param("iii", $idUsuario, $puntaje, $dificultad);
                if ($stmt->execute()) {
        ...
        $missing_fields = [];
        if ($idUsuario === null) $missing_fields[] = 'idUsuario';
