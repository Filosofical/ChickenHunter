...
function gameOver(message) {
 ...
 if (caughtChickens==targetCatchCount){
  ...
  punt=punt*Dificultad;
    if (gameTime !== 0){
    punt=punt*(gameTime/60);} //disminuir el divisor en cada dificultad
...
let mixer; // Mixer del jugador
let actions = {};
let currentAction = null;
let isMoving = false;
let isRunning = false;
let isCatching = false; // Asegúrate que esté declarada si la usas
// let isDefeated = false; // Declaradas pero no usadas actualmente en tu lógica de juego
// let isVictorious = false; // Declaradas pero no usadas
...
if (selectedCharacter === 'Granjero') {
    ...
    loader.load('../escenario/models/dog.glb', (gltf) => {
        ...
        player.position.set(0, 0, 0); // Ajusta la Y si es necesario
        scene.add(player);
        mixer = new THREE.AnimationMixer(player);
        const animations = gltf.animations;
        actions = { // Asegúrate que los nombres de animación coincidan con tu modelo de perro
            idle: mixer.clipAction(findAnimation(animations, "Idle1")), // o el nombre correcto
...
// Gallina
const chickens = [];
const chickenSpeed = 1.5; // Puedes ajustar esta velocidad
const chickenFleeSpeed = 2.5; // Velocidad cuando huyen  modificar en cada diff
...
const chickenCount = 5;  //modificar en cada diff
...
const numTraps = 3; // Define cuántas trampas quieres modificar en cada diff
const trapDamageTime = 10;
...
// Cerca de madera
let entrance; // Asumo que 'entrance' es tu cerca
loader.load('../escenario/models/cerca.glb', (gltf) => {
    entrance = gltf.scene;
    entrance.scale.set(3, 2, 3);
    entrance.position.set(0, 0, 0); // Ajusta la posición para que rodee el área jugable
    scene.add(entrance);
    console.log('Modelo de cerca cargado');
}, undefined, (error) => console.error("Error al cargar el modelo de cerca:", error));
...
loader.load('../escenario/models/trees.glb', (gltf) => {
    ...
    trees.position.set(0, 0, 0); // Ajusta según necesites
    scene.add(trees);
    console.log('Modelo de árboles cargado');
}, undefined, (error) => console.error("Error al cargar el modelo de árboles:", error));
...
document.addEventListener('keydown', (event) => {
    if (isGameOver) return; // No procesar input si el juego terminó
    ...
    if (event.key === 'Shift') {
        isRunning = true;
        // updateAnimationState(); // Se llama en animate()
    }
